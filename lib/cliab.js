...
var results_toString = function(results) { 
    ...
    for (var i = 0 to result.rows.length) {
...
'insert into people (name, shirt) VALUES ("Joe", "Green");', [], nullDataHandler, errorHandler);
    ...
    stmt = "INSERT INTO cliab-objects ( rev, object ) VALUES ( 0 , ? );", [object], nullDataHandler, error_cb_noop);
    ...
    db.transaction(
        function (transaction) {
            transaction.executeSql(stmt, [], result_cb_log, error_cb_log)
        }
    )
...
//------------------------------------------------------------------------------
Database.prototype.createView = function(name, function) {
}
